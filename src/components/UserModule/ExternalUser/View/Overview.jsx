...
const Overview = ({ usersData }) => {
  ...
  const getLogs = (inputs) => {
    ...
    api
      .get(
        `${BASE_URL}/admin/users/logs/${decryptData(id)}?page=${inputs.page}&start_date=${inputs.start_date}&end_date=${inputs.end_date}&user_type=2`
      )
      .then((res) => {
        console.log(res);
        setLoading(false);
  ...
  return (
    <>
      ...
      {/* for tab and mobile */}
      <div className="p-4 bg-light rounded-lg font-audiowide md:block lg:hidden">
        <p className="">Current Session</p>
      ...
      <div className="mt-6 p-4 lg:p-5 bg-light rounded-lg font-audiowide">
        ...
        {/* <div className="mt-4 p-4 rounded-lg bg-tab flex flex-col gap-4">
          {loading ? (
