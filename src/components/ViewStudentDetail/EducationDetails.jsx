                ...
                <div className="grid grid-cols-2 gap-4 lg:flex lg:flex-col  lg:gap-1 ">
                  ...
                  <p className="flex justify-start break-all break-word text-sm md:text-base audio text-white">
                    {student?.education_details[2]?.start_year ? (
                      <>
                        {moment(student?.education_details[2]?.start_year).format(
                          "YYYY"
                        )}
                        /{" "}
                ...
                <div className="grid grid-cols-2 gap-4 lg:flex lg:flex-col  lg:gap-1 ">
                  ...
                  <p className="flex justify-start break-all break-word text-sm md:text-base audio text-white">
                    {student?.education_details[3]?.start_year ? (
                      <>
                        {moment(student?.education_details[3]?.start_year).format(
                          "YYYY"
                        )}
                        /{" "}
      ...
      {student?.asst_exam_sections && student?.asst_exam_sections.length > 0 ? (
        ...
        >
          ...
          <AccordionDetails className={classes.details}>
            <div className="flex flex-col gap-6">
              {
                student?.asst_exam_sections && student?.asst_exam_sections.map((x, index) =>
                  <div key={index} className="bg-[#151929] rounded-lg p-4">
                    <p className="audio text-white mb-1">
                      {index === 1 ? ' ELP test' : student?.level === 'Under Graduation' ? 'Pre SAT/SAT/ACT' : 'GRE/GMAT'
                      } - {x.label || 'NA'}
                    </p>
                    <hr />
                    <div className="mt-4 flex flex-col gap-4 lg:grid lg:grid-cols-4">
                      {
                        x.label && Object.keys(x).filter(k => k !== 'type' && k !== 'label').sort(z => z !== 'overall_score').map(item =>
                          <div key={item} className="grid grid-cols-2 gap-4 lg:flex lg:flex-col  lg:gap-1 ">
                            <p className="text-secondary text-xs">{item.toUpperCase()}</p>
                            <p className="flex justify-start break-all break-word text-sm md:text-base audio text-white">
                              {x[item]}
                            </p>
                          </div>
