...
const Explore = () => {
    ...
    const selectOrDeselect = (item) => {
        ...
        const metaData = { ...meta };
        if (metaData.type_of_degree_category && Array.isArray(metaData.type_of_degree_category)) {
    ...
    const getMetaList = (response) => {
        console.log('getMetaList called with response:', response);
        // axios
        //     .get(`${environment.API_BASE_URL}/prerequisite`)
        //     .then((res) => {

                // Add null check for response
                if (!response || !response.study_destination) {
                    console.error('Invalid response data in getMetaList:', response);
                    return;
                }
                ...
                // Ensure study_destination is an array
                if (!Array.isArray(study_destination)) {
                    console.error('study_destination is not an array:', study_destination);
                    return;
                }
                ...
                for (let item of study_destination) {
                    ...
                    if (flag) {
                        // Handle Next.js imported images - they can be objects or strings
                        item["flag"] = flag.img;
                        item["short_name"] = flag.name;
                    } else {
                        // If no flag match, still keep the country but log for debugging
                        console.warn(`No flag found for country: ${item.name}`);
                    }
                }
                ...
                // Process type_of_degree_category if it exists
                let type_of_degree_category = [];
                if (response.type_of_degree_category && Array.isArray(response.type_of_degree_category)) {
                    type_of_degree_category = response.type_of_degree_category.map((category) => ({
                        ...category,
                        checked: false,
                ...
                response.type_of_degree_category = type_of_degree_category;

                // Only filter out countries without flags if we want strict filtering
                // For now, keep all countries even if they don't have flags
                // response.study_destination = study_destination.filter((x) => x.flag);
                response.study_destination = study_destination;
                console.log('Setting meta with study_destination count:', study_destination.length);
                console.log('Final meta data:', response);
                setMeta(response);
                setFilteredData(response);
    };

    const onSelectDuration = (item) => {
        if (!meta?.study_durations || !Array.isArray(meta.study_durations)) {
            console.error('study_durations is not available or not an array');
            return;
        }
    ...
    function onIndustrySelect(item) {
        if (!meta?.study_industry || !Array.isArray(meta.study_industry)) {
            console.error('study_industry is not available or not an array');
            return;
        }
    ...
    const studyNext = () => {
        if (
            ...
            let isSelected =
                (meta?.type_of_degree_category?.some((x) => x.checked) || false) &&
            ...
            let isSelected = meta?.type_of_degree_category?.some((x) => x.checked) || false;
            if (isSelected) {
                setStep(3);
            } else {
    ...
    const onIntakeSelect = (item) => {
        let isSelected = meta?.study_durations?.some((x) => x.checked) || false;
        ...
        if (
            ...
            if (isSelected && inputs.intake_year && inputs.intake_month) {
                setStep(4);
            } else if (!isSelected || !inputs.intake_year || !inputs.intake_month) {
            ...
            if (inputs.intake_year && inputs.intake_month) {
                ...
                toast.error("Please choose intake month and year");
                return;
            }
        }
    };
    ...
    const onStudyDestination = (item) => {
        if (!meta || !meta.study_destination || !Array.isArray(meta.study_destination)) {
            console.error('Invalid meta.study_destination in onStudyDestination');
            return;
        }
                                                    ...
                                                    <div className="vertical-tab-sec-col">
                                                        ...
                                                        <div className="country-sec">
                                                            <div
                                                                ...
                                                                id="StudyIndustry"
                                                            >
                                                                {(meta?.study_destination && Array.isArray(meta.study_destination) && meta.study_destination.length > 0 ? meta.study_destination : []).map((x, i) => {
                                                                    // Handle flag image - Next.js imports can be objects or strings
                                                                    let flagSrc = null;
                                                                    if (x?.flag) {
                                                                        if (typeof x.flag === 'string') {
                                                                            flagSrc = x.flag;
                                                                        } else if (x.flag?.src) {
                                                                            flagSrc = x.flag.src;
                                                                        } else if (x.flag?.default) {
                                                                            flagSrc = x.flag.default;
                                                                        } else {
                                                                            flagSrc = x.flag;
                                                                        }
                                                                    }
                                                                        ...
                                                                        >
                                                                            <div>
                                                                                {flagSrc ? (
                                                                                    ...
                                                                                    <div className="flag-img" style={{ backgroundColor: '#e0e0e0', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '12px', color: '#666' }}>
                                                                                        {x?.name?.charAt(0)?.toUpperCase() || '?'}
                                                                                    </div>
                                                                                )}

                                                                                <p className={`relative top-2 ${x.checked ? 'text-white' : 'text-[#1E417C]'} `}>
                                                                                    {x.short_name || x.name || 'Country'}
                                                                                </p>
                                                                            </div>
                                                                        </div>
                                                                    );
                                                        ...
                                                        <div className="vertical-tab-content-section w-full">
                                                            <div className="card-view ">
                                                                <div className="study-level">
                                                                    <ul className="">
                                                                        {(meta?.type_of_degree_category && Array.isArray(meta.type_of_degree_category) ? meta.type_of_degree_category : []).map((x) => (
                                                ...
                                                <>
                                                    <div className="vertical-tab-sec-col">
                                                        ...
                                                        {meta?.study_durations &&
                                                            Array.isArray(meta.study_durations) &&
                                                            meta.study_durations.length > 0 &&
                                                            (inputs.level === "Post Graduation" ||
                                                                inputs.level === "Under Graduation") && (
                                                                <>
                                                                    <div className="vertical-tab-header">
