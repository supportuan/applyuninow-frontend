...
import { capitalize, decryptData, encryptData } from "../../../utils/helpers";
...
const EditStaff = () => {
  ...
  const handleSubmit = (e) => {
    ...
    if (!params.user_offer_letter) {
      formData.append("offer_letter", '');
    } else {
      formData.append("offer_letter", params.user_offer_letter);
    }
  ...
  const getSingleUser = () => {
    api
      .get(`${BASE_URL}/admin/users/${decryptData(id)}`)
      .then((res) => {
        ...
        setParams({
          ...
          emg_contact_name: userData.emg_contact_name,
          emg_contact_phone: userData.emg_contact_phone,
          emg_contact_email: userData.emg_contact_email,
          address: userData.address,
          city: userData.city,
          state: userData.state,
          user_image: userData.image,
          user_offer_letter: userData.offer_letter,
        });
      })
      .catch((err) => {
        console.log(err);
  ...
  const update = (payload) => {
    setLoading(true);
    api
      .put(`${BASE_URL}/admin/users/${decryptData(id)}`, payload)
      .then((res) => {
        setLoading(false);
        toast.success(res.data.data.message);
        navigate(`/users/internal/view/${id}`);
      })
      .catch((error) => {
        setLoading(false);
  ...
  return (
    <div className="p-2 lg:p-6 text-white">
      <p className="text-xs">
        <Link to={`/users/internal`} className="mr-1 cursor-pointer">
          Internal Staff
        </Link>{" "}
        {">"}
        <Link
          to={`/users/internal/view/${id}`}
          className="ml-2 mr-1 cursor-pointer"
        >
          View Internal Staff
        </Link>{" "}
        {">"}
        <span className="ml-2">Edit Internal Staff</span>
      </p>
      <p className="text-2xl font-semibold mb-4">Edit Internal Staff</p>
              ...
              <div className="flex flex-col w-full mt-2 lg:mt-0">
                <div className="mb-4">
                  ...
                  <FileUpload
                    ...
                    bgcolor="#262938"
                    imageUrl={params?.user_offer_letter}
                  />
                </div>
                <div>
