...
export const UserListingTable = () => {
  ...
  const getAllRoles = () => {
    api
      .get(`${BASE_URL}/admin/users/roles?user_type=internal`)
      .then((res) => {
  ...
  const getUsers = (inputs) => {
    ...
    api
      .get(
        `${BASE_URL}/admin/users?role_id=${inputs.role_id}&active=${inputs.active}&search_key=${inputs.search_key}&created_from=${inputs.created_from}&created_to=${inputs.created_to}&page=${inputs.page}&user_type=internal`
      )
      .then((res) => {
        setLoading(false);
                  ...
                  <tr className="my-4 bg-tab users-list" key={uuid()}>
                    ...
                    <td>
                      <div className="bg-tab py-5 rounded-r-xl flex space-x-4 items-center justify-center">
                        <Link to={`/users/edit-user/${item.id}?type=internal`}>
